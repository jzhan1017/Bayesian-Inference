...
public class Assignment extends LinkedHashMap<RandomVariable,Object> {
	...
	public boolean isConsistent(Assignment other) {
		for(RandomVariable i: this.keySet()){
			for(RandomVariable j : other.keySet()){
				if((i.getName()).equals(j.getName()) && !(this.get(i)).equals(other.get(j))){
					return false;
				}
			}
		}
