...
public class BayesianNetwork {
	...
	public Set<RandomVariable> getParents(RandomVariable X) {
		trace("BayesinNetwork.getParents: X=" + X);
		ArraySet<RandomVariable> parents = new ArraySet<RandomVariable>();
//				Node node = getNodeForVariable(X);
//				for (Node parentNode: node.parents) {
//					parents.add(parentNode.variable);
//					trace("BayesinNetwork.getParents: " + parentNode.variable);
//				}
//				return parents;
			for (RandomVariable rv: getVariableListTopologicallySorted()) {
				if (!getChildren(rv).contains(X)) {
					parents.add(rv);
				}
			}
			parents.add(X);
